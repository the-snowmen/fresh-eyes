...
4. **Apply.** Run `/fresh-eyes:apply`, which implements the FIX-now items on `fresh-eyes/fixes-<version>`,
...
- **Cost.** A full loop spends the browser-driving persona agents twice (initial + re-review) plus the
  code edits. For a quick decision-only pass, run `/fresh-eyes:triage` alone.
