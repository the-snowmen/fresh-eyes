...
the decision doc before any code changes), run `review`, `decide`, and `apply` separately instead.

## What it chains

```
review  →  decide  →  apply  →  (apply auto-re-reviews)
...
- `decide` reads those, writes `_decisions-<version>.md`
...
1. **Review (unless fresh).** If `./fresh-eyes/` has no review for today's version, run `/fresh-eyes:fresh-eyes-review`
   first. If a current-version review already exists, skip straight to decide (don't burn the browser
   agents twice).
2. **Decide.** Run `/fresh-eyes:fresh-eyes-decide` to produce `_decisions-<version>.md`.
3. **Checkpoint (announce, don't block).** Print the decision tally (X fix / Y defer / Z won't-fix) and the
...
- **Cost.** A full loop spends the browser-driving persona agents twice (initial + re-review) plus the
  code edits. For a quick decision-only pass, run `/fresh-eyes:fresh-eyes-decide` alone.
- Same isolation model as the parts: `review` is walled off; `decide`/`apply` are the internal maintainer.
